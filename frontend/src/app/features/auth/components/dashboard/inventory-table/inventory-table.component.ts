...
export class InventoryTableComponent implements OnInit {
  products: { id: number, product_name: string, category_name: string,product_image?: string , quantity_in_stock: number, unit_price: number, status: string, vendors: string, isSelected?: boolean }[] = [];
  ...
  constructor(private productService: ProductService,private toastr: ToastrService, private http: HttpClient,private fb:FormBuilder) {
  ...
  loadVendorsAndCategories(): void {
    this.http.get<any>('http://localhost:3000/api/vendors-and-categories').subscribe({
      next: (data) => {
 ...
 onImageSelect(event: any): void {
    ...
    addProducts() {
      if(this.addProductForm.valid)
      {
        ...
        if(this.addProductForm.value.productImage!=null)
        {
          ...
          this.http.post('http://localhost:3000/api/v1/profile/upload-product', formData).subscribe({
            next: (response: any) => {
              console.log('Profile picture uploaded successfully', response);
              //this.userProfile = response;
              this.selectedImage=response.thumbnailUrl;
              this.productService.addProduct(productData,this.selectedImage).subscribe(
                {
                  next:(response) => {
  ...
  onDragOver(event: DragEvent): void {
    ...
    getDisplayedPages(): number[] {
      const range: number[] = [];
      const rangeSize = 3; // Number of pages to display around the current page
